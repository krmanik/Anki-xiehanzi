import{s as e}from"./BXWjpja8.js";import{r as t}from"./D_7tN5S8.js";import{a as n}from"./CFFI4KRD.js";var r=null;function i(){return r??=fetch(`${e}/data/radicals/index.json`).then(e=>{if(!e.ok)throw Error(`Radical data unavailable (${e.status})`);return e.json()}),r}var a=null;function o(){return a??=fetch(`${e}/data/radicals/strokes.json`).then(e=>e.ok?e.json():Promise.reject(Error(`strokes.json ${e.status}`))).catch(()=>fetch(`${e}/data/hanzi-writer-data.json`).then(e=>e.json())),a}var s=null;function c(){return s??=fetch(`${e}/data/radicals/deck.json`).then(e=>e.ok?e.json():null).then(e=>e?.editions?e:null).catch(()=>null),s}function l(t){return`${e}/data/radicals/glyphs/${t}`}function u(e){return(e??``).normalize(`NFD`).replace(/[̀-ͯ]/g,``).toLowerCase()}function d(e,t){let n=t.trim();if(!n)return e;let r=u(n).replace(/\s+/g,``),i=/^\d+$/.test(n)?Number(n):null;return e.filter(e=>i!==null&&e.number===i||e.char===n||e.variants.includes(n)||e.simplified.includes(n)||e.traditional.includes(n)||e.colloquial?.term.includes(n)||e.kana.includes(n)||e.hangul.includes(n)||e.examples.some(e=>e.char===n)?!0:u([e.pinyin,e.meaning,e.hanviet,e.romaji,e.romaja,e.colloquial?.english??``].join(` `)).replace(/\s+/g,``).includes(r))}var f=[{value:`number`,label:`Kangxi order`},{value:`strokes`,label:`Stroke count`},{value:`frequency`,label:`Most productive first`},{value:`pinyin`,label:`Pinyin A–Z`}];function ee(e,t){let n=[...e];switch(t){case`strokes`:return n.sort((e,t)=>e.strokes-t.strokes||e.number-t.number);case`frequency`:return n.sort((e,t)=>t.frequency-e.frequency||e.number-t.number);case`pinyin`:return n.sort((e,t)=>u(e.pinyin).localeCompare(u(t.pinyin))||e.number-t.number);default:return n.sort((e,t)=>e.number-t.number)}}function te(e){return[...new Set(e.map(e=>e.strokes))].sort((e,t)=>e-t)}function ne(e){return n(e??``)}function p(e){return e>=300?`Very common`:e>=100?`Common`:e>=30?`Moderate`:e>=5?`Uncommon`:`Rare`}var m=[{value:`recognize`,name:`Recognize`,label:`Recognition — glyph on the front`},{value:`write`,name:`Write`,label:`Writing — produce it from the meaning`}],h={edition:`free`,cards:[`recognize`,`write`],audio:!0,strokeOrder:!0,readings:!0,colloquial:!0,examples:!0,glyphs:!1,asWord:!1,toneColors:!0,fieldToggles:!1},re={...h,edition:`premium`,glyphs:!0,asWord:!0,fieldToggles:!0};function g(e=`premium`,t={}){let n={...e===`free`?h:re,...t,edition:e},r=m.map(e=>e.value).filter(e=>n.cards.includes(e));return{...n,cards:r.length?r:[`recognize`],glyphs:e===`premium`&&n.glyphs,asWord:e===`premium`&&n.asWord,fieldToggles:e===`premium`&&n.fieldToggles}}var _=(e=`premium`)=>typeof e==`string`?g(e):e,v={premium:{recognize:`1969669521`,write:`1969669523`},free:{recognize:`1969669522`,write:`1969669524`}};v.premium.recognize;var y=(e=`premium`,t=`recognize`)=>v[e][t],b=(e=`premium`,t=`recognize`)=>`Kangxi Radical${e===`free`?` Free`:``} ${t===`write`?`Write`:`Recognize`} - (Anki-xiehanzi)`;function x(e=`premium`,t){let n=`Anki xiehanzi::Kangxi Radicals${e===`free`?` (Free)`:``}`;return t?`${n}::${t===`write`?`Write`:`Recognize`}`:n}var S=(e,t=`mp3`)=>`xhz-radical-${e.number}.${t}`,C=e=>`_xhzr-${e}`,w=`Number.Radical.Variants.Simplified.Traditional.Strokes.StrokeLabel.Pinyin.Meaning.Colloquial.ColloquialPinyin.ColloquialMeaning.HanViet.Japanese.Korean.Frequency.Productivity.Band.Examples.Evolution.Regional.Audio.StrokeData.Zhuyin.AsWord.Unicode.KangxiForm`.split(`.`),T=e=>String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`);function E(e){return(e??``).split(/(\s+)/).map(e=>e.trim()?`<span class="t${n(e)}">${T(e)}</span>`:T(e)).join(``)}function ie(e){return e?.length?`<ul class="examples">${e.map(e=>`<li class="ex"><span class="ex-char">${T(e.char)}</span><span class="ex-body"><span class="ex-pinyin">${E(e.pinyin)}</span>`+(e.zhuyin?`<span class="ex-zhuyin">${T(e.zhuyin)}</span>`:``)+`<span class="ex-meaning">${T(e.meaning)}</span></span>`+(e.band?`<span class="chip chip--freq">${T(e.band)}</span>`:``)+`</li>`).join(``)}</ul>`:``}function ae(e){return e?.meaning?`<span class="word-pinyin">${E(e.pinyin)}</span><span class="word-meaning">${T(e.meaning)}</span>`+(e.band?`<span class="chip chip--freq">${T(e.band)}</span>`:``):``}function D(e,t){return e?.length?`<div class="glyph-row glyph-row--${t}">${e.map(e=>{let t=T(e.script??e.region??``);return`<figure class="glyph"><img src="${T(C(e.file))}" alt="${T(e.label)}"><figcaption><span class="glyph-cn">${t}</span><span class="glyph-en">${T(e.label)}</span></figcaption></figure>`}).join(``)}</div>`:``}function O(e,t){let n=[e,t].map(e=>(e??``).trim()).filter(Boolean);return n.length?n.map((e,t)=>t===0&&n.length>1?`<span class="r-native">${T(e)}</span>`:`<span class="r-roman">${T(e)}</span>`).join(` `):``}function oe(e,t={}){let n=t.audio===void 0?S(e):t.audio;return{Number:String(e.number),Radical:e.char,Variants:e.variants.join(` `),Simplified:e.simplified.join(` `),Traditional:e.traditional.join(` `),Strokes:String(e.strokes),StrokeLabel:`${e.strokes} stroke${e.strokes===1?``:`s`}`,Pinyin:E(e.pinyin),Meaning:T(e.meaning),Colloquial:T(e.colloquial?.term??``),ColloquialPinyin:E(e.colloquial?.pinyin??``),ColloquialMeaning:T(e.colloquial?.english??``),HanViet:T(e.hanviet),Japanese:O(e.kana,e.romaji),Korean:O(e.hangul,e.romaja),Frequency:e.frequency?String(e.frequency):``,Productivity:k(e.frequency),Band:e.frequency?A(e.frequency):``,Examples:ie(e.examples),Evolution:D(e.evolution,`evolution`),Regional:D(e.compare,`regional`),Audio:n?`[sound:${n}]`:``,StrokeData:t.strokeData??``,Zhuyin:T(e.zhuyin??``),AsWord:ae(e.word),Unicode:T(e.unicode??``),KangxiForm:e.kangxiForm?`${T(e.kangxiForm)} <span class="muted">U+${e.kangxiForm.codePointAt(0).toString(16).toUpperCase()}</span>`:``}}function k(e){return e?`${e} character${e===1?``:`s`}`:``}function A(e){return e?e>=300?`Very common`:e>=100?`Common`:e>=30?`Moderate`:`Rare`:``}function j(e){let t=[`Xiehanzi`,`Kangxi_Radical`,`Strokes::${String(e.strokes).padStart(2,`0`)}`],n=A(e.frequency)||`Rare`;return t.push(`Productivity::${n.replace(/\s+/g,`_`)}`),t}function M(e,t=`premium`,n=`recognize`){let r=`xiehanzi-radical:${y(t,n)}:${e.number}:${e.char}`,i=2166136261,a=522970236;for(let e=0;e<r.length;e++){let t=r.charCodeAt(e);i=Math.imul(i^t,16777619),a=Math.imul(a^t,16777619)}return(i>>>0).toString(36).padStart(7,`0`)+(a>>>0).toString(36).padStart(7,`0`)}var N=`_xhz-hanzi-writer.js`,P=`<script>
(function () {
  ...
  function withEngine(run) {
    ...
    function inject(source) {
      ...
      else { tag.src = '${N}'; tag.onload = tag.onerror = done; }
      document.head.appendChild(tag);
      if (source) done();
    }

    try {
      fetch('${N}').then(function (res) {
...
<\/script>`;function F(e,t=!0){let n=`<div class="writer-wrap" ${J(`outline`)}>
  <div id="xhz-writer" class="writer tappable" ${J(`grid`)} data-char="{{text:Radical}}" data-mode="${e}"></div>
...
${P}`;return t?`{{#StrokeData}}\n${n}\n{{/StrokeData}}`:n}var I={play:`<path d="M8 5.5v13l11-6.5z"/>`,replay:`<path d="M12 5V2L8 6l4 4V7a5 5 0 1 1-5 5H5a7 7 0 1 0 7-7z"/>`,pencil:`<path d="m4 16.2 9.4-9.4 4.1 4.1L8.1 20H4zM15 5.2l1.6-1.6a1.2 1.2 0 0 1 1.7 0l2.4 2.4a1.2 1.2 0 0 1 0 1.7L19.1 9.3z"/>`,eye:`<path d="M12 5.5c-4.5 0-8 4-9 6.5 1 2.5 4.5 6.5 9 6.5s8-4 9-6.5c-1-2.5-4.5-6.5-9-6.5zm0 11a4.5 4.5 0 1 1 0-9 4.5 4.5 0 0 1 0 9zm0-2.2a2.3 2.3 0 1 0 0-4.6 2.3 2.3 0 0 0 0 4.6z"/>`,sliders:`<path d="M4 7h9v2H4zm12 0h4v2h-4zM4 15h4v2H4zm7 0h9v2h-9z"/><path d="M13.5 5.5h2v5h-2zm-5 8h2v5h-2z"/>`,more:`<path d="M12 8a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm0 6a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm0 6a2 2 0 1 1 0-4 2 2 0 0 1 0 4z"/>`,close:`<path d="M6 6l12 12M18 6L6 18" fill="none" stroke="currentColor" stroke-width="1.8"/>`,github:`<path d="M12 .5C5.7.5.9 5.4.9 11.6c0 4.9 3.2 9.1 7.6 10.5.6.1.8-.2.8-.5v-1.9c-3.1.7-3.8-1.5-3.8-1.5-.5-1.3-1.2-1.6-1.2-1.6-1-.7.1-.7.1-.7 1.1.1 1.7 1.2 1.7 1.2 1 1.7 2.6 1.2 3.2.9.1-.7.4-1.2.7-1.5-2.5-.3-5.1-1.2-5.1-5.5 0-1.2.4-2.2 1.1-3-.1-.3-.5-1.4.1-3 0 0 .9-.3 3.1 1.1a10.6 10.6 0 0 1 5.6 0c2.1-1.4 3.1-1.1 3.1-1.1.6 1.6.2 2.7.1 3 .7.8 1.1 1.8 1.1 3 0 4.3-2.6 5.2-5.1 5.5.4.4.8 1 .8 2.1v3.1c0 .3.2.6.8.5 4.4-1.4 7.6-5.6 7.6-10.5C23.1 5.4 18.3.5 12 .5z"/>`},L=`https://github.com/krmanik/Anki-xiehanzi`,R=e=>`<svg class="ico" viewBox="0 0 24 24" aria-hidden="true">${I[e]}</svg>`,z=`var h=document.getElementById('xhz-audio'),e=h&amp;&amp;h.getElementsByTagName('*')[0];if(e)e.tagName==='AUDIO'?e.play():e.click()`,B=e=>`if(window.xhzWriterAction)window.xhzWriterAction('${e}')`,V={audio:{label:`Play audio`,icon:`play`,call:z,field:`Audio`},replay:{label:`Replay the strokes`,icon:`replay`,call:B(`replay`),field:`StrokeData`},practice:{label:`Practise writing it`,icon:`pencil`,call:B(`practice`),field:`StrokeData`},hint:{label:`Show me a stroke`,icon:`eye`,call:B(`hint`),field:`StrokeData`}},H=`<button type="button" class="bar-btn bar-btn--tool cog tappable" aria-label="Show or hide parts of the card" title="Show or hide parts of the card" onclick="document.querySelector('.card-body').classList.toggle('xhz-panel')">${R(`sliders`)}</button>`,U=`<button type="button" class="bar-btn bar-btn--tool more-btn tappable" aria-label="Look it up elsewhere" title="Look it up elsewhere" onclick="document.querySelector('.card-body').classList.toggle('xhz-more')">${R(`more`)}</button>`;function W(e,t={}){let n=e.map(e=>{let t=V[e];return`{{#${t.field}}}<button type="button" class="bar-btn tappable" aria-label="${t.label}" title="${t.label}" onclick="${t.call}">${R(t.icon)}</button>{{/${t.field}}}`}).join(`
    `);if(!e.length&&!t.cog&&!t.more)return``;let r=e.length?`<div class="bar-actions" ${J(`buttons`)}>\n    ${n}\n  </div>`:`<div class="bar-actions"></div>`,i=t.more?`<div class="bar-side bar-side--right" ${J(`buttons`)}>\n    ${U}\n  </div>`:`<div class="bar-side bar-side--right"></div>`;return`<div class="bar">
  <div class="bar-side bar-side--left">${t.cog?`\n    ${H}\n  `:``}</div>
  ${r}
  ${i}
</div>`}var se=`<aside class="more" aria-label="Look it up elsewhere">
  <div class="panel-head">
    <span class="panel-title">Look it up</span>
    <button type="button" class="panel-close tappable" aria-label="Close" onclick="document.querySelector('.card-body').classList.remove('xhz-more')">${R(`close`)}</button>
  </div>
...
</aside>`,G=`{{#Audio}}<div id="xhz-audio" class="audio-holder">{{Audio}}</div>{{/Audio}}`,K=e=>[`var r=document.querySelector('.card-body'),k='${e}';`,`r.classList.toggle('xhz-h-'+k,!this.checked);`,`try{`,`var s=r.getAttribute('data-side')||'front',n='xhz.hide2.'+s,`,`l=(localStorage.getItem(n)||'').split(',').filter(Boolean),i=l.indexOf(k);`,`if(this.checked){if(i>-1)l.splice(i,1)}else if(i<0)l.push(k);`,`localStorage.setItem(n,l.join(','))`,`}catch(e){}`,e===`outline`?`if(window.xhzWriterAction)window.xhzWriterAction('outline');`:``,`if(window.xhzSync)window.xhzSync()`].join(``);function ce(e){if(!e.length)return``;let t=e.map(e=>`    <label class="panel-row tappable" data-row="${e}"><input class="tappable" type="checkbox" checked value="${e}" onchange="${K(e)}"><span>${q[e]}</span></label>`).join(`
`);return`<aside class="panel" aria-label="Show or hide parts of the card">
  <div class="panel-brand">
    ...
    <button type="button" class="panel-close tappable" aria-label="Close" onclick="document.querySelector('.card-body').classList.remove('xhz-panel')">${R(`close`)}</button>
  </div>
  <div class="panel-head">
...
${t}
  ...
  <div class="panel-foot">
    <a class="panel-gh tappable" href="${L}">${R(`github`)}<span>View on GitHub</span></a>
  </div>
</aside>`}var le=`<script>
(function () {
...
<\/script>`,q={strokes:`Stroke order`,grid:`Grid lines`,outline:`Character outline`,glyph:`Glyph`,pinyin:`Pinyin`,zhuyin:`Zhuyin`,meaning:`Meaning`,meta:`Kangxi line`,forms:`Other forms`,name:`Teaching name`,readings:`Readings`,word:`As a word`,evolution:`Evolution`,regional:`Regional forms`,examples:`Examples`,buttons:`Buttons`,codes:`Codepoints`},ue={pinyin:[`.ex-pinyin`,`.coll-pinyin`,`.word-pinyin`],zhuyin:[`.ex-zhuyin`],meaning:[`.ex-meaning`,`.word-meaning`]},J=e=>`data-xhz="${e}"`,Y=(e,t,n,r=``,i)=>{let a=`<section class="block${r?` ${r}`:``}"${i?` ${J(i)}`:``}>
  <h2>${e}</h2>
  <div class="block-body">${t}</div>
</section>`;return n?`{{#${n}}}${a}{{/${n}}}`:a},de=`<table class="readings">
    ...
    </table>`;function fe(e,t=new Set){return[`glyph`,`pinyin`,...e.asWord?[`zhuyin`]:[],`meaning`,`meta`,`forms`,...e.colloquial?[`name`]:[],...e.readings?[`readings`]:[]].filter(e=>!t.has(e))}function pe(e,t=new Set){let n=e=>!t.has(e),r=n(`forms`)?`{{#Variants}}<div class="forms" ${J(`forms`)}><span class="forms-label">also written</span>{{Variants}}</div>{{/Variants}}
  {{#Simplified}}<div class="forms" ${J(`forms`)}><span class="forms-label">simplified</span>{{Simplified}}</div>{{/Simplified}}
  {{#Traditional}}<div class="forms" ${J(`forms`)}><span class="forms-label">traditional</span>{{Traditional}}</div>{{/Traditional}}`:``,i=e.colloquial&&n(`name`)?`{{#Colloquial}}<div class="colloquial" ${J(`name`)}>
  ...
  </div>{{/Colloquial}}`:``,a=e.readings&&n(`readings`)?`<div class="ident-readings" ${J(`readings`)}>
    <h2>Readings</h2>
    ${de}
  </div>`:``,o=[n(`pinyin`)?`<span class="pinyin" ${J(`pinyin`)}>{{Pinyin}}</span>`:``,e.asWord&&n(`zhuyin`)?`{{#Zhuyin}}<span class="zhuyin" ${J(`zhuyin`)}>{{Zhuyin}}</span>{{/Zhuyin}}`:``].join(``),s=[n(`glyph`)?`    <span class="ident-glyph" ${J(`glyph`)}>{{Radical}}</span>`:``,o&&`    <span class="ident-say">\n      ${o}\n    </span>`].filter(Boolean),c=[s.length&&`  <div class="ident-head">\n${s.join(`
`)}\n  </div>`,n(`meaning`)&&`  <div class="ident-meaning" ${J(`meaning`)}>{{Meaning}}</div>`,n(`meta`)&&`  <div class="ident-meta" ${J(`meta`)}>Kangxi {{Number}} · {{StrokeLabel}}{{#Productivity}} · {{Productivity}}{{/Productivity}}{{#Band}} · {{Band}}{{/Band}}</div>`,r&&`  ${r}`,i&&`  ${i}`,a&&`  ${a}`].filter(Boolean);return c.length?[`<div class="ident">`,...c,`</div>`].join(`
`):``}function X(e,t,n=new Set){let r=e.strokeOrder&&!n.has(`strokes`)?`{{#StrokeData}}<section class="block block--grid" ${J(`strokes`)}>
  <h2>Stroke order</h2>
  <div class="block-body block-body--center">
${F(`animate`,!1)}
  </div>
</section>{{/StrokeData}}`:``,i=pe(e,n);return[r?`<div class="duo">\n${[r,i].filter(Boolean).join(`
`)}\n</div>`:i,t,e.asWord&&!n.has(`word`)&&Y(`As a word`,`{{AsWord}}`,`AsWord`,`block--word`,`word`),e.glyphs&&!n.has(`evolution`)&&Y(`Evolution <span class="h2-cn">字源演变</span>`,`{{Evolution}}`,`Evolution`,`block--evolution`,`evolution`),e.glyphs&&!n.has(`regional`)&&Y(`Regional forms <span class="h2-cn">字形对比</span>`,`{{Regional}}`,`Regional`,`block--regional`,`regional`),e.examples&&!n.has(`examples`)&&Y(`Examples`,`{{Examples}}`,`Examples`,`block--examples`,`examples`),e.asWord&&!n.has(`codes`)&&`<div class="foot" ${J(`codes`)}>
  {{#Unicode}}<span>{{Unicode}}</span>{{/Unicode}}
  {{#KangxiForm}}<span>Kangxi radical form {{KangxiForm}}</span>{{/KangxiForm}}
</div>`].filter(Boolean).join(`

`)}var me=`  <div class="glyph-main" ${J(`glyph`)}>{{Radical}}</div>
  <div class="kangxi" ${J(`meta`)}>Kangxi radical {{Number}} · {{StrokeLabel}}</div>`;function Z(e,t=new Set){return[...e.strokeOrder?[`strokes`,`grid`,`outline`]:[],`glyph`,`pinyin`,...e.asWord?[`zhuyin`]:[],`meaning`,`meta`,`forms`,...e.colloquial?[`name`]:[],...e.readings?[`readings`]:[],...e.asWord?[`word`]:[],...e.glyphs?[`evolution`,`regional`]:[],...e.examples?[`examples`]:[],...e.asWord?[`codes`]:[]].filter(e=>!t.has(e))}var Q={recognize:[`glyph`,`meta`],write:[`meaning`,`pinyin`,`meta`]};function he(e=`premium`){let t=_(e),n=[...t.audio?[`audio`]:[],...t.strokeOrder?[`replay`,`practice`]:[]],r=e=>t.fieldToggles?`${ce(e)}\n${le}`:``,i=[...Z(t).filter(e=>e!==`codes`),...n.length?[`buttons`]:[],...t.asWord?[`codes`]:[]],a=`<div class="card-body back" data-side="back">
${X(t,W(n,{cog:t.fieldToggles,more:!0}))}
${r(i)}
${se}
${t.audio?G:``}
</div>`,o=e=>{if(!t.fieldToggles)return{html:``,parts:[],hidden:``};let n=new Set([...e,`strokes`,`grid`,`outline`,`buttons`]),r=Z(t,n);return r.length?{html:`<div class="extras">\n${X(t,``,n)}\n</div>`,parts:r,hidden:r.map(e=>` xhz-h-${e}`).join(``)}:{html:``,parts:r,hidden:``}},s=o(Q.recognize),c=o(Q.write),l={recognize:`<div class="card-body front front--recognize${s.hidden}" data-side="recognize">
${me}
${W([],{cog:t.fieldToggles})}
${s.html}
${r([`glyph`,`meta`,...s.parts])}
</div>`,write:`<div class="card-body front front--write${c.hidden} xhz-h-outline" data-side="write">
  <div class="prompt">
    <span class="prompt-meaning" ${J(`meaning`)}>{{Meaning}}</span>
    <span class="prompt-pinyin" ${J(`pinyin`)}>{{Pinyin}}</span>
  </div>
  <div class="kangxi" ${J(`meta`)}>Kangxi radical {{Number}} · {{StrokeLabel}}</div>
${F(`quiz`)}
${W([`hint`,`replay`],{cog:t.fieldToggles})}
${c.html}
${r([`meaning`,`pinyin`,`meta`,`grid`,`outline`,`buttons`,...c.parts])}
</div>`};return t.cards.map(e=>({name:m.find(t=>t.value===e).name,qfmt:l[e],afmt:a}))}function ge(e=`premium`){let t=e=>w.indexOf(e),n={recognize:[t(`Radical`)],write:[t(`Meaning`),t(`StrokeData`)]};return _(e).cards.map((e,t)=>[t,`all`,n[e]])}var $=`
.card {
  --bg: #eef1f8;
  --surface: #ffffff;
  --fg: #1b1c22;
...
${Object.keys(q).filter(e=>e!==`grid`&&e!==`outline`).map(e=>`${[`[data-xhz='${e}']`,...ue[e]??[]].map(t=>`.card-body.xhz-h-${e} ${t}`).join(`,
...
`;function ve(e){let t=[new Set,new Set(Q.recognize),new Set(Q.write)].map(t=>fe(e,t)).filter(e=>e.length).map(e=>`.card-body${e.map(e=>`.xhz-h-${e}`).join(``)} .ident`);return`${[...new Set(t)].join(`,
`)} { display: none; }`}function ye(e=`premium`){let t=_(e),n=`${$}\n${ve(t)}\n`;return t.toneColors?n:n+_e}function be(e){let t=w.indexOf(`Number`),n=e.exec(`SELECT id, flds FROM notes`);if(!n.length)return;let r=e.prepare(`UPDATE notes SET sfld = ? WHERE id = ?`),i=e.prepare(`UPDATE cards SET due = ? WHERE nid = ?`);e.run(`BEGIN`);for(let[e,a]of n[0].values){let n=Number(String(a).split(``)[t])||0;r.run([n,e]),i.run([n,e])}e.run(`COMMIT`),r.free(),i.free()}export{ne as C,p as S,te as T,d as _,oe as a,o as b,be as c,M as d,g as f,f as g,he as h,S as i,ye as l,j as m,m as n,y as o,ge as p,w as r,b as s,N as t,x as u,l as v,ee as w,i as x,c as y};